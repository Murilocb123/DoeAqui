    ...
    <div>
        <x-form actionEdit="/doacao/edit/{{$doacao['id']}}" actionCreate="/doacao/create" id="{{$doacao['id']}}" titulo="doação" redirect="/doacao">
           ...
           <x-select value="{{$doacao['id_doacao_destino']}}" name="id_doacao_destino" label="Endereco" 
                :options="$EnderecoListSelect" visualizacao="{{$visualizacao}}"/>    
            ...
            <x-select value="{{$doacao['status']}}" name="status" label="Status" visualizacao="{{$visualizacao}}"
            :options="[
           ...
           <x-input value="{{$doacao['caminho_imagem']}}" columnsmd="5"  type="file" name="imagem" label="imagem"
                placeholder="imagem" visualizacao="{{$visualizacao}}"/>  {{-- precisa adicionar multiple para varias fotos --}}
